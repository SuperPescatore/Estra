...
<h1>Elimina utente</h1>
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        // Verifica se il parametro di rimozione dell'utente è stato inviato
        if (isset($_POST["user_id"])) {
            $userIDToRemove = $_POST["user_id"];

            // Query per verificare se l'ID utente esiste
            $checkUserQuery = "SELECT * FROM admin_user WHERE user = '$userIDToRemove'";
            $checkUserResult = $conn->query($checkUserQuery);

            if ($checkUserResult->num_rows > 0) {
                // ID utente trovato, esegui la rimozione
                $deleteUserQuery = "DELETE FROM admin_user WHERE user = '$userIDToRemove'";

                if ($conn->query($deleteUserQuery) === TRUE) {
                    echo "<script>showPopup('Utente rimosso con successo.', '')</script>";
                } else {
                    echo "<script>showPopup('Errore durante la rimozione dell\'utente:', '" . $conn->error . "')</script>";
                }
            } else {
                // ID utente non trovato
                echo "<script>showPopup('Impossibile rimuovere l\'utente.', 'L\'ID utente non esiste.')</script>";
            }
        }
